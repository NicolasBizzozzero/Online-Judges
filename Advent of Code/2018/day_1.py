...
PATH_FILE_INPUT = "day_1_input.txt"


def parse_data(file_path, encoding="utf8"):
    with open(PATH_FILE_INPUT, encoding=encoding) as file:
        return list(map(lambda l: int(l[:-1]), file.readlines()))
...
def second_half(data):
    ...
    current_frequency = 0
    for frequency in cycle(data):
        current_frequency += frequency
        if current_frequency not in frequencies_seen:
