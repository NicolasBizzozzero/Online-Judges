...
PATH_FILE_INPUT = "day_2_input.txt"


def parse_data(file_path, encoding="utf8") -> List[str]:
    with open(PATH_FILE_INPUT, encoding=encoding) as file:
        return list(map(lambda l: l[:-1], file.readlines()))


def first_half(data) -> int:
    count_2 = sum(map(lambda id: _id_contains_identics_chars(id, 2), data))
    count_3 = sum(map(lambda id: _id_contains_identics_chars(id, 3), data))
    return count_2 * count_3


def second_half(data) -> str:
    for id1 in data:
        for id2 in data:
            diff = sum(_bitwise_nand_str(id1, id2))
            if diff == 1:
                answer = [str1 for str1, str2 in zip(id1, id2) if str1 == str2]
                return ''.join(answer)


def _id_contains_identics_chars(id, number_of_identics_chars) -> bool:
    return number_of_identics_chars in Counter(id).values()


def _bitwise_and_str(str1, str2) -> List[bool]:
    return [c1 == c2 for c1, c2 in zip(str1, str2)]


def _bitwise_nand_str(str1, str2) -> List[bool]:
    return [c1 != c2 for c1, c2 in zip(str1, str2)]


def main():
    data = parse_data(PATH_FILE_INPUT)
    print("First half solution  :", first_half(data))
    print("Second half solution :", second_half(data))
